...
This document specifies the cryptographic design and implementation of derived passwords. Derived passwords are deterministic, high-entropy, per-label passwords rendered from a [derived key](derived-key.md) — a 256-bit key binding a label, a memorized master passphrase and a YubiKey HMAC-SHA1 challenge-response — two-factor password derivation with nothing stored anywhere. The source code ([src/utilities/crypto/derivedPassword.ts](../../src/utilities/crypto/derivedPassword.ts) and [src/utilities/crypto/derivedKey.ts](../../src/utilities/crypto/derivedKey.ts)) is the ground truth for this document, and the reference vectors in [tests/derivedPassword.test.ts](../../tests/derivedPassword.test.ts) pin the scheme.
...
Derived passwords extend Superbacked with stateless password derivation. A password is never stored — it exists only while it is computed, from a label naming its purpose and two factors: a memorized master passphrase and the non-exportable HMAC-SHA1 secret inside a YubiKey. The same three inputs produce the same password on any machine, so there is no vault to steal, sync or lose — and guessing the master passphrase requires the YubiKey hardware.

The two factors meet in the derived key, whose derivation — Argon2d stretching, keyed challenge, YubiKey response and HKDF combination — is specified in the [derived key technical documentation](derived-key.md), not here. This document specifies how a password is rendered from that key.

## Terminology

- **Character class**: one of lowercase, uppercase, digits and special characters.
- **Derived key**: 256-bit key binding label, master passphrase and YubiKey response (see the [derived key technical documentation](derived-key.md)).
- **Determinism input**: any input whose change silently derives a different password — the label, the master passphrase, the YubiKey slot secret, Paranoid mode and the scheme version.
...
1. Derives a 256-bit key from the label, master passphrase and YubiKey (see the [derived key technical documentation](derived-key.md))
...
- **Info**: `superbacked-derived-password` followed by fixed-width 32-bit big-endian counter
- **Output**: 64 bytes per counter (two SHA-256 output blocks — enough for a default-length password in a single expansion despite rejection losses) — incrementing the counter yields an unbounded deterministic stream

### Security characteristics

- **Domain separation**: The `superbacked-derived-password` context separates password rendering from every other use of a derived key (for example YubiKey-protected archive key derivation, which consumes the key through its own HKDF context) — a rendered password reveals nothing about the key
...
- **Capacity**: Default 16-character password carries ~102 bits (past the 100-bit threshold KeePassXC rates excellent) — short enough to type manually on air-gapped devices, while effective strength is bounded by the passphrase, deliberately the weakest input

## Security model

The two-factor security model — what each combination of leaked material allows — lives with the derived key scheme in the [derived key technical documentation](derived-key.md). Password-specific limitations:
...
- **Determinism inputs**: The scheme version and [Paranoid mode](../../src/shared/kdfProfiles.ts) each derive a different password from the same label and passphrase — every derivation prints both, and re-deriving with a forgotten flag silently produces a different password, not an error
...
- **Loss of YubiKey**: Without a backup of the secret or a second YubiKey provisioned with it, all derived passwords are unrecoverable
- **No verifier**: No fingerprint or checksum of the passphrase is ever displayed or stored — a mistyped passphrase silently derives a different password (use `--confirm-passphrase` when creating a password)

## Command-line interface

```console
superbacked derive-password [options] [label]
```
...
The root `--paranoid` flag derives at the paranoid [KDF profile](../../src/shared/kdfProfiles.ts) and prices the strength gate at it (see the [passphrase strength technical documentation](passphrase-strength.md)). Derivation is stateless, so the mode is a determinism input on par with the label — deriving without it silently produces a different password — which is why every derivation prints the scheme version and mode (for example `Derived using scheme v1 (paranoid)`).

### Derivation workflow

1. Run `superbacked derive-password`.
2. Enter a label.
3. Enter the master passphrase (hidden prompt or piped via stdin).
4. Touch the YubiKey if the slot requires touch.
5. The app copies the password to the clipboard, clearing it after 10 seconds or as soon as enter is pressed (or prints it with `--print`).
