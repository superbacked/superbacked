...
This document specifies the cryptographic design and implementation of derived keys. A derived key is a deterministic 256-bit key derived from a label, a memorized master passphrase and a YubiKey HMAC-SHA1 challenge-response — two-factor key derivation with nothing stored anywhere. The source code ([src/utilities/crypto/derivedKey.ts](../../src/utilities/crypto/derivedKey.ts) and [src/utilities/yubikey/otp.ts](../../src/utilities/yubikey/otp.ts)) is the ground truth for this document, and the reference vectors in [tests/derivedKey.test.ts](../../tests/derivedKey.test.ts) pin the scheme.
...
Derived keys extend Superbacked with stateless key derivation: instead of storing key material, consumers re-derive it on demand from a label and two factors — a memorized master passphrase and the non-exportable HMAC-SHA1 secret inside a YubiKey. The same label, passphrase and YubiKey always produce the same key, on any machine, with no vault, sync or backup required. The label binds every derivation to its purpose, giving every label an independent key. Statelessness is what distinguishes this primitive from the [passphrase key](passphrase-key.md) scheme protecting blocks and standalone archives: those store a random salt, so their derivation binds it directly, while derived keys must derive their salts from the label.

The YubiKey second factor is required — no hardware-less variant exists — yet the scheme still treats the device as an untrusted black box: a malicious or compromised device can never weaken the output below the security of a passphrase-only derivation.
...
- **Master key**: 256-bit key derived from the master passphrase and a label-bound salt using Argon2d.
...
1. Stretches the master passphrase into a 256-bit master key using Argon2d at the active [KDF profile](../../src/shared/kdfProfiles.ts)
...
Every stage is a pure function — no randomness and no stored state — so derivation is deterministic. The scheme is frozen: changing any constant, cost parameter or construction silently changes every derived key — and with it every derived password and wallet.
...
**Algorithm**: Argon2d, at the standard [KDF profile](../../src/shared/kdfProfiles.ts) — or the paranoid profile under Paranoid mode
...
- **Variant**: Argon2d is used everywhere in Superbacked — blocks, archives and derived keys share the variant and the [profile registry](../../src/shared/kdfProfiles.ts) — maximizing offline brute-force resistance through fully data-dependent memory access. RFC 9106 recommends Argon2id as a general-purpose default to hedge cache side channels during legitimate derivation; Superbacked instead assumes a side-channel adversary on a derivation host is capable of direct capture, which no variant survives and optimizes for the attack that defines its threat model — offline brute force of leaked material
...
The response is computed inside the YubiKey using HMAC-SHA1 keyed with the slot secret (see `calculateHmacSha1` in [src/utilities/yubikey/otp.ts](../../src/utilities/yubikey/otp.ts)).
...
Computing a response requires physical touch by default — add `--no-touch` to compute responses without touch (weaker). Challenge-response requires YubiKey firmware 2.2 or later. The generated secret is displayed once so it can be backed up or provisioned to another YubiKey — the secret cannot be read back from the YubiKey, so losing its only copy makes every derived key unrecoverable. Slots provisioned with other tools (for example Yubico Authenticator) remain compatible.
...
- **Possession factor**: The 160-bit slot secret is written once and physically non-exportable thereafter — it never exists on any computer after slot provisioning
...
A hardware-less variant would be single factor: leaked derived material would become offline-attackable, with Argon2d as the only remaining wall — and for derived Bitcoin wallets the public blockchain itself would be the verification oracle, letting anyone grind label and passphrase guesses against on-chain activity without any leak at all. No such variant exists: every derivation mixes in the YubiKey response, and hardware redundancy (a second YubiKey provisioned with the same secret, backed up in a block or blockset) is the recovery mechanism.
...
| YubiKey slot secret and leaked material | Offline passphrase guessing, memory-hard (Argon2d) with per-label dictionaries                     |
| Passphrase alone                        | Nothing — computing the response requires the YubiKey                                              |
...
- **Loss of YubiKey**: Without a backup of the secret or a second YubiKey provisioned with it, every derived key is unrecoverable
...
YubiKey-protected blocks and standalone archives do not consume this scheme — they store a salt, so they bind it from the first step through the [passphrase key](passphrase-key.md) scheme instead. Both schemes share slot provisioning and the wire protocol above.
