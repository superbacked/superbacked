...
This document specifies the cryptographic design and implementation of derived Bitcoin wallets. Derived Bitcoin wallets are deterministic BIP39 wallets rendered from a [derived key](derived-key.md) — a 256-bit key binding a label, a memorized master passphrase and a YubiKey HMAC-SHA1 challenge-response — two-factor wallet derivation with nothing stored anywhere. The mnemonic is the root artifact; the extended keys and receive addresses are projections of it. The source code ([src/utilities/crypto/derivedBitcoinWallet.ts](../../src/utilities/crypto/derivedBitcoinWallet.ts), [src/utilities/crypto/bip84.ts](../../src/utilities/crypto/bip84.ts) — the shared BIP84 projections — and [src/utilities/crypto/derivedKey.ts](../../src/utilities/crypto/derivedKey.ts)) is the ground truth for this document, and the reference vectors in [tests/derivedBitcoinWallet.test.ts](../../tests/derivedBitcoinWallet.test.ts) and [tests/bip84.test.ts](../../tests/bip84.test.ts) pin the scheme.
...
- **Label**: memorized identifier a wallet is derived for (for example `hotwallet`).
...
Every stage is a pure function — no randomness and no stored state — so derivation is deterministic. The rendering is frozen: changing any constant or construction silently changes every derived wallet — as does any change to the derived key scheme beneath it. A wallet can control funds, so unlike a password it can never be rotated away from a mistake.
...
- **Determinism inputs**: The word count and Paranoid mode each derive a different wallet from the same label and passphrase — every derivation prints both, and re-deriving with a forgotten flag silently produces an empty wallet, not an error
- **Loss of YubiKey**: Without a backup of the secret or a second YubiKey provisioned with it, a derived wallet — and the funds it controls — is unrecoverable
...
```console
superbacked derive-bitcoin-wallet [options] [label]
```

Every derivation opens with the host-exposure warning and requires the full word `yes` to continue — never assumed, and refused without a controlling terminal. Label prompting, stdin passphrase behavior and the strength gate match [derive-password](derived-password.md); under [Paranoid mode](../../src/shared/kdfProfiles.ts) the gate is priced at the paranoid profile and the mode is a determinism input printed at every derivation.

The command is public by default, secret on demand: the extended public key (and addresses, when asked) print to stdout, while the mnemonic or extended private key materialize only under `--reveal`.
...
1. Run `superbacked derive-bitcoin-wallet`.
2. Confirm the host-exposure warning by typing `yes`.
3. Enter a label.
4. Enter the master passphrase (hidden prompt or piped via stdin).
5. Touch the YubiKey if the slot requires touch.
6. The app prints the extended public key (and addresses with `--addresses`), stating the scheme version, Paranoid mode, derivation path and word count on stderr — the path is fixed, printed for cross-verification in wallet software.
7. With `--reveal`, the app copies the mnemonic or extended private key to the clipboard, clearing it after 10 seconds or as soon as enter is pressed (or prints it with `--print`).

To verify a wallet before funding it, import the mnemonic into wallet software (for example Electrum) and confirm it reports the same extended public key and addresses the command printed.
