...
Legacy standalone archives are the scheme standalone archives were built on before the current one: a passphrase-encrypted tar archive with a headerless layout, keyed by the raw Argon2d stretched key. The current scheme added the [probe block](../../../src/utilities/crypto/schemeHeader.ts), the version-probe key and the YubiKey second factor; this scheme carries none of them.
