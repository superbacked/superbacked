...
Superbacked converts the estimate into a score by pricing zxcvbn’s guess count against the calibrated million-dollar attack budget at the active [KDF profile](scheme-registry-technical-documentation.md) (see below):
...
New passphrases must score a strength of at least **50**, priced at the [KDF profile](scheme-registry-technical-documentation.md) the passphrase will stretch under — the threshold holds the attack time constant across profiles, not the entropy, so Paranoid mode admits proportionally weaker passphrases as a deliberate, user-owned trade. The gate applies wherever a **new** passphrase is accepted:
