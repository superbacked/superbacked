...
This document specifies the cryptographic design and implementation of the derived Bitcoin wallet feature in Superbacked. Derived Bitcoin wallets are deterministic BIP39 wallets rendered from a [derived key](derived-key-technical-documentation.md) — a 256-bit key binding a memorized master passphrase, a label and a YubiKey HMAC-SHA1 challenge-response — two-factor wallet derivation with nothing stored anywhere. The mnemonic is the root artifact; the extended keys and receive addresses are projections of it. The source ([src/utilities/crypto/derivedBitcoinWallet.ts](../src/utilities/crypto/derivedBitcoinWallet.ts) and [src/utilities/crypto/derivedKey.ts](../src/utilities/crypto/derivedKey.ts)) is the ground truth for this document, and the reference vectors in [tests/derivedBitcoinWallet.test.ts](../tests/derivedBitcoinWallet.test.ts) pin the scheme.
...
The two factors meet in the derived key, whose derivation — Argon2d stretching, keyed challenge, YubiKey response and HKDF combination — is specified in the [derived key technical documentation](derived-key-technical-documentation.md), not here. This document specifies how a wallet is rendered from that key.

## Terminology

- **Derivation path**: BIP32 path locating the account within the wallet (for example `m/84'/0'/0'`).
...
- **Determinism input**: any input whose change silently derives a different wallet — the label, the master passphrase, the YubiKey slot secret, Paranoid mode, the scheme version, the word count and the derivation path.
...
- **Info**: `superbacked-derived-mnemonic-v1-24` or `superbacked-derived-mnemonic-v1-12`
- **Output**: 32 bytes (24 words) or 16 bytes (12 words)

The frozen context string predates the command’s name — scheme identities never move with product naming.

### Security characteristics

- **Domain separation**: The `superbacked-derived-mnemonic-v1-` context separates wallet entropy from every other use of a derived key (for example [derived password](derived-password-technical-documentation.md) rendering) — a wallet and a password derived from the same label never share bytes
...
The BIP39 seed roots a BIP32 hierarchy from which three projections derive at the requested derivation path:

- **Extended public key**: the account public key, serialized with [SLIP-132](https://github.com/satoshilabs/slips/blob/master/slip-0132.md) version bytes labeled by the path’s purpose — `zpub` for native segwit (`m/84'`), `ypub` for wrapped segwit (`m/49'`), `xpub` otherwise
- **Receive addresses**: bech32 P2WPKH addresses at `path/0/0` through `path/0/count-1` — native segwit (`m/84'`) mainnet only, as other purposes use other address encodings
...
- `--addresses <count>`: Print first receive addresses (`m/84'` paths only, maximum `100`)
...
- `--derivation-path <path>`: BIP32 derivation path (default `m/84'/0'/0'`)
...
- `--reveal <secret>`: Materialize `mnemonic` or `zprv`, copied to clipboard with the same clearing, guarding and platform behavior as [derive-password](derived-password-technical-documentation.md)
...
6. App prints the extended public key (and addresses with `--addresses`), stating the scheme version, Paranoid mode, derivation path and word count on stderr
