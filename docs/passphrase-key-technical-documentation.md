...
This document specifies passphrase key derivation — the scheme [standalone archives](standalone-archive-technical-documentation.md) and [blocks](block-technical-documentation.md) share to turn a memorized passphrase and a stored salt into a 256-bit key: the passphrase stretched with Argon2d at a [KDF profile](scheme-registry-technical-documentation.md) alone, or, when the optional YubiKey second factor is requested, mixed with a YubiKey HMAC-SHA1 challenge-response. The scheme also derives the probe key through which version 2 artifacts declare their version. The source ([src/utilities/crypto/passphraseKey.ts](../src/utilities/crypto/passphraseKey.ts) and [src/utilities/yubikey/otp.ts](../src/utilities/yubikey/otp.ts)) is the ground truth for this document, and the reference vectors in [tests/passphraseKey.test.ts](../tests/passphraseKey.test.ts) pin the scheme.
...
- **KDF profile**: named, frozen Argon2d cost parameters (see the [scheme registry technical documentation](scheme-registry-technical-documentation.md)).
...
1. Stretches the passphrase and the salt into a 256-bit stretched key using Argon2d at the artifact’s [KDF profile](scheme-registry-technical-documentation.md) — without a YubiKey, this is the key
...
At restoration the profile is discovered by trial (see the [scheme registry technical documentation](scheme-registry-technical-documentation.md)) — one stretch (and, with a YubiKey, one touch) per candidate profile, expanded into both the probe key and the consumer key.
...
The profile names the Argon2d cost — legacy for every version 1 artifact, v2 standard for new artifacts and v2 paranoid under Paranoid mode (see the [scheme registry technical documentation](scheme-registry-technical-documentation.md) for the parameter table and rules).

### Security characteristics

- **Memory hardness**: Each passphrase guess costs the profile’s memory across its passes in memory-bound work (64 MiB × 80 at the v2 standard profile), defending against GPU and ASIC-based attacks
...
Version 2 artifacts declare their version through a probe (see the [scheme registry technical documentation](scheme-registry-technical-documentation.md)), keyed by an HKDF sibling of the consumer key:
...
- **[Standalone archives](standalone-archive-technical-documentation.md)**: `computeArchiveKeys` freezes info `encryption-key-v1` for the encryption key and `version-probe-v1` for the probe key — one stretch expanded into both (`--yubikey` or the app switch)
