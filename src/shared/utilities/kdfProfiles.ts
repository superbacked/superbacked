...
// (see docs/scheme-registry-technical-documentation.md), so every row is
...
// Creation default as of v2 — 8× legacy attack cost at the same 64 MiB,
// so the restore hardware floor is unchanged
export const v2StandardKdfProfile: KdfProfile = Object.freeze({
  memoryKiB: 65536,
  passes: 80,
  parallelism: 4,
})

// Opt-in (Settings and --paranoid, creation and restore) — 10× the v2
// attack cost, and 1 GiB of memory raises the minimum attack hardware
// 16-fold. The same gigabyte is a restore floor: restoring requires the
// mode enabled and ≥1 GiB of free memory
export const v2ParanoidKdfProfile: KdfProfile = Object.freeze({
  memoryKiB: 1048576,
  passes: 50,
  parallelism: 4,
})
