...
# never anticipated it. Completeness comes from the complain-mode
...
if [ "$(journalctl -b -k | grep -c "callbacks suppressed")" -gt 0 ]; then
  printf "%s\n" "Error: rate-limit suppression detected — the log is incomplete (harvest on a complain-mode image, which disables rate limiting at boot)" >&2
  exit 1
fi

journalctl -b -k \
  | grep -E 'apparmor="(ALLOWED|DENIED)"' \
  | { if [ -n "${profile}" ]; then grep -E "(profile|label)=\"${profile}\b"; else cat; fi }
